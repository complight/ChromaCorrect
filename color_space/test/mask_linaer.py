...
def third_channel_mask(resolution = [1920,1080], distance_from_screen = 500, pixel_pitch = 0.311, gaze=None):
    ...
    for rad in distance_range:
                mask = (distance_from_center * pixel_pitch   <= rad)              
                mask = mask.swapaxes(0,1)
                current_angle = math.degrees(math.atan(rad/distance_from_screen))   
    ...
    print(math.degrees(math.atan(rad/distance_from_screen))/max_degree) 
...
color_1_mask , color_1_mask_cropped, color_2_mask_cropped, luminance_mask_cropped = third_channel_mask([1920,1080],300,0.311,[0.5,0.5])
